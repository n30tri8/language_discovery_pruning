...
def load_pawsx_italian(base_dir: str, sample_size: int, split: str):
    if split == "dev":
        file = os.path.join(base_dir, "xglue_dataset", "PAWSX", "it-translated", "validation.jsonl")
    elif split == "test":
        file = os.path.join(base_dir, "xglue_dataset", "PAWSX", "it-translated", "test.jsonl")
    else:
        raise ValueError(f"Unsupported split: {split}")
    data = []
    with open(file, encoding="utf-8") as f:
